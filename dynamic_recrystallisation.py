...
def dynamic_rx(rho_critical, rho_curr, n_rx_prev,
               r_rx_prev, r_g_prev, x_prev, x_c_prev, time_step, temp_curr,
               temp_strain_rate, temp_strain, D_def):

    mean_free_path_dis_cr = 1/((inputdata.fitting_params['C_1'] / D_def) +
                               inputdata.fitting_params['C_2'] * math.sqrt(rho_critical))
    mean_free_path_dis = 1/((inputdata.fitting_params['C_1'] / D_def) +
                            inputdata.fitting_params['C_2'] * math.sqrt(rho_curr))
    
    'Calculation of R_xc'
    subpart1 = (3*mat_const.calc_burgers_vector(temp_curr)*mat_const.calc_gb_mobility2_drx(temp_curr)*
                mat_const.calc_dislocation_line_energy(temp_curr)) / (4 * inputdata.M * inputdata.fitting_params['C_10'])
    ...
    fragment1 = (inputdata.fitting_params['C_1'] / D_def) + (inputdata.fitting_params['C_2'] * math.sqrt(rho_curr))
