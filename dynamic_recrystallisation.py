...
def dynamic_rx(rho_critical, rho_curr, n_rx_prev,
    ...
    kf = 0.4e6
    nucleation_rate = kf * temp_strain_rate * p_r * (1 - x_prev) / (
                n_d * D_def * mat_const.calc_burgers_vector(temp_curr) *
                mean_free_path_dis)
    ...
    drg_dt = rho_curr * psi * mat_const.calc_dislocation_line_energy(temp_curr) * mat_const.calc_gb_mobility2_drx(
        temp_curr)
    ...
    fragment2 = r_rx / (
                mat_const.calc_burgers_vector(temp_curr) * mat_const.calc_dislocation_line_energy(temp_curr) * rho_curr)
