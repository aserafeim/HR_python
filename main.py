...
for i in range(1, max_row - 1):
    ...
    if strain_rate_initial > 0:
        ...
        while time_curr <= pass_interval:
            ...
            storage_dict['stress'].append(mat_const.calculate_stress(storage_dict['temperature'][j], strain_rate_temp, rho_temp_var))
        ...
        while time_curr <= pass_interval:
            ...
            storage_dict['temperature'].append(storage_dict['temperature'][j] +
                                               variable_pass['temp_rate_of_change'] * time_step)
            'Calcukation of D_def with strain = 0, because the effect of changing grain size cannot be taken into account'
            D_def = storage_dict['d_mean'][0] * math.exp(-2 * 0 / math.sqrt(3))
